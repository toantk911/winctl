...
winctl.FindWindows(win => win.isVisible() && win.getTitle()).then(windows => {
	...
	windows.sort((a,b) => a.getTitle().localeCompare(b.getTitle())).forEach(window => console.log(" - %s [classname=%s, pid=%d, hwnd=%d, description=%s]", window.getTitle(), window.getClassName(), window.getPid(), window.getHwnd(), window.getDescription()));
});
/* -->
Visible windows:
...
winctl.Events.addListener("active-window", function(now, prev) {
	console.log("Changed active window to: %s [prev=%s]", now.getDescription(), prev.getDescription());
});
